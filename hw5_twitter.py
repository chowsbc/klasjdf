...
def make_request(baseurl, params):
    ...
    dict
        ...
        a dictionary
    '''
    return requests.get(baseurl, params=params, auth=oauth)
    pass
...
def make_request_with_cache(baseurl, hashtag, count):
    ...
    dict
        ...
        JSON
    '''
    if (baseurl + hashtag) in CACHE_DICT:
        twitter_return = open_cache()
    else:
        r = requests.get(baseurl, params={'q': hashtag, 'count': count}, auth=oauth)
        twitter_return = json.loads(r.text)
        CACHE_DICT[baseurl] = twitter_return
...
def find_most_common_cooccurring_hashtag(tweet_data, hashtag_to_ignore):
    ...
    tweet_list = tweet_data['statuses']
    master_taglist = []
    histogram = {}
    poptweet_total = 0
    commontweet = ''
    ...
    for t in tweet_list:
        list_of_hashtags = t['entities']['hashtags']

        linelist = []
        for hashtag in list_of_hashtags:
            linelist.append(hashtag['text'])

            if hashtag_to_ignore in linelist:
                for word in linelist:
                    if word == hashtag_to_ignore:
                        pass
                    else:
                        master_taglist.append(word)
        else:
            pass
    for tag in master_taglist:
        if tag in histogram:
            histogram[tag] += 1
        else:
            histogram[tag] = 1
    for key in histogram:
        if histogram[key] > poptweet_total:
            poptweet_total = histogram[key]
            commontweet = key
